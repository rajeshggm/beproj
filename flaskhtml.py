...
class PythiaDemo:
  ...
  def predict(self, url, question):
    with torch.no_grad():
      global detectron_features
      global resnet_features
  ...
  def predictmm(self, url, question):
    with torch.no_grad():
...
print("model lodede")
#image_text="dog.jpg"
#question_text=" what is the color of dog"
...
def upload_filee():
   if request.method == 'POST':
      ...
      print (imgname)
      clear_output()#new
      question_text=" what is there?"#new
      scores, predictions = demo.predict(imgname, question_text)#new
      print (predictions[0])#new
      shutil.copy(imgname, './static')
      
      full_filename = os.path.join(app.config['UPLOAD_FOLDER'], imgname)
      return render_template("index.html", name = full_filename)
...
#image = Image.open(image_path)
      ...
      scores, predictions = demo.predictmm(imgname, question_text)
      scores = [score * 100 for score in scores]
      val1 = round(scores[0],2)
      val2 = round (scores[2],2)
      ans = str(val1 )+ "%  " +predictions[0]
      ans2 = str(val2) + "%  " +predictions[1]
      print (ans)
      torch.cuda.empty_cache()
      full_filename = os.path.join(app.config['UPLOAD_FOLDER'], imgname)
                
      return render_template("index.html",ans = ans, name = full_filename,ans2= ans2)
      #return 'file uploaded successfully'
